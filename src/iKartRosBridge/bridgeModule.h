...
class BridgeModule: public RFModule
{
    ...
    bool respond(const Bottle& command, Bottle& reply)
    {
        ...
        {
            bridge_thr->setGoal(command.get(1).asDouble(),command.get(2).asDouble(),command.get(3).asDouble());
            reply.addString("ack");
            return  true;
        }
        else
        ...
        if (command.get(0).asString()=="set")
        {
            ...
            {
                bridge_thr->setGoal(command.get(2).asDouble(),command.get(3).asDouble(),command.get(4).asDouble());
                reply.addString("ack");
                return  true;
            }
        }
